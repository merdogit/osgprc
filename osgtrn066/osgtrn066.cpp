...
public:
    ...
    void operator()(osg::Node *, osg::NodeVisitor *nv) override
    {
        ...
        else
        {
            ...
            if (dirDesired.length2() < 1e-8f)
                dirDesired.set(1, 0, 0);
...
public:
    ...
    void createView(const ViewInfo &info, int w = 900, int h = 650)
    {
    ...
    void duplicateView(osgViewer::View *src, const std::string &labelSuffix = " (Copy)")
    {
        if (!src)
            return;
        ...
        dup->getCamera()->setViewMatrix(view);
        dup->getCamera()->setClearColor(cam->getClearColor());
        osg::ref_ptr<osgGA::OrbitManipulator> manip = new osgGA::OrbitManipulator;
        manip->setVerticalAxisFixed(false);
        manip->setByMatrix(view);
...
protected:
    void drawUi() override
    {
        ...
        if (ImGui::Button("New Screen"))
        {
            ...
            {
            case 0:
                info = {"Fighter Side", ViewPreset::FighterSide, osg::Vec3(0, -80, 0), osg::Vec3(0, 0, 0), osg::Vec3(0, 0, -1)};
                break;
            case 1:
                info = {"Fighter Front", ViewPreset::FighterFront, osg::Vec3(60, 0, 0), osg::Vec3(0, 0, 0), osg::Vec3(0, 0, -1)};
                break;
            case 2:
                info = {"Missile Top", ViewPreset::MissileTop, osg::Vec3(0, -80, -30), osg::Vec3(0, 0, 0), osg::Vec3(0, 0, -1)};
                break;
            case 3:
                info = {"Missile Rear", ViewPreset::MissileRear, osg::Vec3(80, 0, -30), osg::Vec3(0, 0, 0), osg::Vec3(0, 0, -1)};
                break;
            default:
                info = {"Custom", ViewPreset::Custom, eye, center, up};
                break;
            }
            if (_vm)
                _vm->createView(info);
        ...
        if (ImGui::Button("Duplicate Current"))
        {
            if (_vm && !_vm->entries().empty())
                _vm->duplicateView(_vm->entries().back().view.get());
        }
        ImGui::Separator();
        if (_vm)
        {

...
const osg::Quat CESSNA_BASIS(-0.700883, 0.713223, 0.0062242, -0.00611652); // NED frame

const osg::Quat TYPHOON_BASIS(0.0122325, -0.700803, 0.713142, -0.0124479); // Slight offset
const osg::Quat F14_BASIS(0.0, 0.713223, -0.700883, 0.0);                   // ENU frame
const osg::Quat MISSILE_BASIS(0, 0, 1, 0);                                  // tip forward

// Pick the aircraft basis here:
const osg::Quat PLANE_BASIS = F14_BASIS;
                                           ...
                                           const osg::Vec3 &pNext)
{
    ...
    // Yaw & pitch from forward direction
    float yaw = atan2f(fwd.y(), fwd.x());
    float pitch = asinf(-fwd.z());

    // Roll based on curvature
    osg::Vec3 prevDir = p - pPrev;
    prevDir.normalize();
    osg::Vec3 turnAxis = prevDir ^ fwd; // curve normal
    float turnStrength = turnAxis.length();
    float bankSign = (turnAxis.z() > 0.0f) ? -1.0f : 1.0f;
    float roll = bankSign * turnStrength * 1.8f; // tune intensity
...
public:
    ...
    void operator()(osg::Node *, osg::NodeVisitor *nv) override
    {
        ...
        osg::Quat q = orientationFromTrajectory(pPrev, p, pNext) * PLANE_BASIS;

        mt->setMatrix(osg::Matrix::rotate(q) * osg::Matrix::translate(p));

        if (trail.valid())
        {
            osg::Vec3 bodyForward = q * osg::Vec3(1, 0, 0);
            trail->add(p - bodyForward * std::fabs(gTailOffset));
        }
...
public:
    ...
    void operator()(osg::Node *, osg::NodeVisitor *nv) override
    {
        ...
        osg::Quat q = orientationFromTrajectory(p, p, p2) * MISSILE_BASIS;
...
protected:
    void drawUi() override
    {
        ...
        ImGui::Text("Aircraft now banks correctly (F14, Cessna, Typhoon).");
        ImGui::End();
    }

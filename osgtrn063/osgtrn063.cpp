...
int main()
{
    ...
    const std::string baseDir  = "/home/murate/Documents/SwTrn/OsgPrc/osgtrn063/";
    ...
    // ============================================================
    // ========== COMPOSITE VIEWER WITH 4 CAMERAS ================
    // ============================================================
    ...
    compViewer->setRealizeOperation(new ImGuiInitOperation); // ImGui global init here ✅

    const int winX = 100, winY = 100, winW = 1200, winH = 900;
    ...
    // === Create four views ===
    auto createView = [&](const osg::Vec4 &viewport, const osg::Vec3d &eye, const osg::Vec3d &center, const osg::Vec3d &up)
    {
        osg::ref_ptr<osgViewer::View> view = new osgViewer::View;
        view->getCamera()->setGraphicsContext(gc.get());
        view->getCamera()->setViewport(new osg::Viewport(
            viewport.x() * winW, viewport.y() * winH,
            viewport.z() * winW, viewport.w() * winH));

        view->getCamera()->setProjectionMatrixAsPerspective(45.0, (double)viewport.z() / viewport.w(), 1.0, 10000.0);
        view->getCamera()->setClearColor(osg::Vec4(0.1f, 0.1f, 0.15f, 1.0f));
        view->setSceneData(root.get());

        // Attach ImGui handler per-view (optional, can remove for secondary views)
        view->addEventHandler(new ImGuiControl(trailAircraft.get(), trailMissile.get()));
    ...
    // Viewport definitions: (x, y, widthFraction, heightFraction)
    osg::ref_ptr<osgViewer::View> v1 = createView({0.0, 0.5, 0.5, 0.5}, osg::Vec3(0,-80,0), osg::Vec3(0,0,0), osg::Vec3(0,0,-1)); // fighter side
    osg::ref_ptr<osgViewer::View> v2 = createView({0.5, 0.5, 0.5, 0.5}, osg::Vec3(60,0,0),  osg::Vec3(0,0,0), osg::Vec3(0,0,-1)); // fighter front
    osg::ref_ptr<osgViewer::View> v3 = createView({0.0, 0.0, 0.5, 0.5}, osg::Vec3(0,-80,-30), osg::Vec3(0,0,0), osg::Vec3(0,0,-1)); // missile top
    osg::ref_ptr<osgViewer::View> v4 = createView({0.5, 0.0, 0.5, 0.5}, osg::Vec3(80,0,-30), osg::Vec3(0,0,0), osg::Vec3(0,0,-1)); // missile rear

    // ============================================================
    // ========== SINGLE / MULTI VIEW SWITCH =====================
    // ============================================================
    static bool showFourViews = true;

    class ViewToggleHandler : public osgGA::GUIEventHandler
    {
    public:
        ViewToggleHandler(osgViewer::CompositeViewer *cv,
                          const std::vector<osg::ref_ptr<osgViewer::View>> &views,
                          int w, int h)
            : _cv(cv), _views(views), _winW(w), _winH(h) {}

        bool handle(const osgGA::GUIEventAdapter &ea, osgGA::GUIActionAdapter &) override
        {
            if (ea.getEventType() == osgGA::GUIEventAdapter::KEYDOWN && ea.getKey() == 'v')
            {
                showFourViews = !showFourViews;
                for (size_t i = 0; i < _views.size(); ++i)
                {
                    osg::Camera *cam = _views[i]->getCamera();
                    if (showFourViews)
                    {
                        if (i == 0) cam->setViewport(new osg::Viewport(0, _winH/2, _winW/2, _winH/2));
                        if (i == 1) cam->setViewport(new osg::Viewport(_winW/2, _winH/2, _winW/2, _winH/2));
                        if (i == 2) cam->setViewport(new osg::Viewport(0, 0, _winW/2, _winH/2));
                        if (i == 3) cam->setViewport(new osg::Viewport(_winW/2, 0, _winW/2, _winH/2));
                    }
                    else
                    {
                        // only first view full screen
                        if (i == 0) cam->setViewport(new osg::Viewport(0, 0, _winW, _winH));
                        else cam->setViewport(new osg::Viewport(0, 0, 0, 0));
                    }
                }
                std::cout << (showFourViews ? "Switched to 4-view layout\n" : "Switched to single-view layout\n");
                return true;
            }
            return false;
        }
    ...
    private:
        ...
        std::vector<osg::ref_ptr<osgViewer::View>> _views;
        int _winW, _winH;
    };

    std::vector<osg::ref_ptr<osgViewer::View>> views = {v1, v2, v3, v4};
    for (auto &v : views)
        v->addEventHandler(new ViewToggleHandler(compViewer.get(), views, winW, winH));

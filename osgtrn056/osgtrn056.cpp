...
const osg::Quat PLANE_BASIS = TYPHOON_BASIS;
                                           ...
                                           const osg::Vec3 &pNext)
{
    ...
    // Yaw and pitch from forward vector
    float yaw = atan2f(fwd.y(), fwd.x());
    float pitch = asinf(-fwd.z());

    // --- Bank (roll) based on curvature of the path ---
    osg::Vec3 prevDir = p - pPrev;
    prevDir.normalize();
    osg::Vec3 turnAxis = prevDir ^ fwd; // cross product = curve normal
    float turnStrength = turnAxis.length();
    float bankSign = (turnAxis.z() > 0.0f) ? -1.0f : 1.0f;
    float roll = bankSign * turnStrength * 1.8f; // adjust 1.8f for roll intensity
...
{
public:
    F14CB(osg::MatrixTransform *m, Trail *t, const TrajData *d)
        : mt(m), trail(t), data(d) {}
    void operator()(osg::Node *, osg::NodeVisitor *nv) override
    {
        ...
        if (trail.valid())
            trail->add(p - ((PLANE_BASIS * q) * osg::Vec3(1, 0, 0)) * gTailOffset);
        traverse(mt.get(), nv);
    }
...
protected:
    void drawUi() override
    {
        ...
        ImGui::Text("F-14 now banks naturally into turns.");
        ImGui::End();
    }
...
int main()
{
    ...
    osg::ref_ptr<osg::Node> acraft = osgDB::readNodeFile(dataPath + "typhoon-no-land-gear.ac");
